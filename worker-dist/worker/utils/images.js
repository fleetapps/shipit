...
(function (ImageType) {
    ImageType["SCREENSHOTS"] = "screenshots";
    ImageType["UPLOADS"] = "uploads";
})(ImageType || (ImageType = {}));
export async function uploadImageToCloudflareImages(env, image, type, bytes) {
...
export function getPublicUrlForR2Image(env, r2Key) {
    const protocol = getProtocolForHost(env.CUSTOM_DOMAIN);
    const base = `${protocol}://${env.CUSTOM_DOMAIN}`;
    const url = `${base}/api/${r2Key}`;
    return url;
}
export async function uploadImageToR2(env, image, type, cfImagesUrl, bytes) {
    const data = bytes ?? base64ToUint8Array(image.base64Data);
...
export async function uploadImage(env, image, type) {
...
function sanitizeBase64Data(dataUrl) {
    return dataUrl.replace(/^data:image\/\w+;base64,/, '');
}
export async function hashImageB64url(dataUrl) {
...
export async function imageToBase64(env, image) {
    try {
