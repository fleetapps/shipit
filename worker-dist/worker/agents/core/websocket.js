...
export function handleWebSocketMessage(agent, connection, message) {
    try {
        ...
        switch (parsedMessage.type) {
            ...
            case WebSocketMessageRequests.USER_SUGGESTION:
                ...
                agent.handleUserInput(parsedMessage.message, parsedMessage.images).catch((error) => {
...
export function handleWebSocketClose(connection) {
    logger.info(`WebSocket connection closed: ${connection.id}`);
}
export function broadcastToConnections(agent, type, data) {
    const connections = agent.getWebSockets();
    for (const connection of connections) {
        sendToConnection(connection, type, data);
    }
}
export function sendToConnection(connection, type, data) {
    try {
...
export function sendError(connection, errorMessage) {
