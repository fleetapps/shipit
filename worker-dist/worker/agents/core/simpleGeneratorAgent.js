...
import { CurrentDevState, MAX_PHASES } from './state';
...
export class SimpleCodeGeneratorAgent extends Agent {
    static MAX_COMMANDS_HISTORY = 10;
    static PROJECT_NAME_PREFIX_MAX_LENGTH = 20;
    projectSetupAssistant;
    stateManager;
    fileManager;
    codingAgent = new CodingAgentInterface(this);
    deploymentManager;
    git;
    previewUrlCache = '';
    templateDetailsCache = null;
    // In-memory storage for user-uploaded images (not persisted in DO state)
    pendingUserImages = [];
    generationPromise = null;
    currentAbortController;
    deepDebugPromise = null;
    deepDebugConversationId = null;
    ...
    _logger;
    initLogger(agentId, sessionId, userId) {
    ...
    constructor(ctx, env) {
        ...
        this.stateManager = new StateManager(() => this.state, (s) => this.setState(s));
     ...
     */
    async initialize(initArgs, ..._args) {
        ...
        const projectName = generateProjectName(blueprint?.projectName || templateInfo.templateDetails.name, generateNanoId(), SimpleCodeGeneratorAgent.PROJECT_NAME_PREFIX_MAX_LENGTH);
        ...
        await this.fileManager.saveGeneratedFiles(filesToSave, 'Initialize project configuration files');
     ...
     */
    async updateBootstrapScript(commandsHistory) {
    ...
    */
    getConversationState(id = DEFAULT_CONVERSATION_SESSION_ID) {
        const currentConversation = this.state.conversationMessages;
        const rows = this.sql `SELECT * FROM full_conversations WHERE id = ${id}`;
        let fullHistory = [];
        ...
        const compactRows = this.sql `SELECT * FROM compact_conversations WHERE id = ${id}`;
        let runningHistory = [];
    ...
    rechargePhasesCounter(max_phases = MAX_PHASES) {
    ...
    createNewIncompletePhase(phaseConcept) {
        this.setState({
            ...this.state,
    ...
    async queueUserRequest(request, images) {
    ...
    async launchStateMachine(reviewCycles) {
     ...
     */
    async executePhaseImplementation(phaseConcept, staticAnalysis, userContext) {
        try {
    ...
    async executeDeepDebug(issue, toolRenderer, streamCb, focusPaths) {
        const debugPromise = (async () => {
            try {
                ...
                const filesIndex = operationOptions.context.allFiles
                    .filter((f) => !focusPaths?.length ||
                    focusPaths.some((p) => f.filePath.includes(p)));
                const runtimeErrors = await this.fetchRuntimeErrors(true);
                const dbg = new DeepCodeDebugger(operationOptions.env, operationOptions.inferenceContext);
                const out = await dbg.run({ issue, previousTranscript }, { filesIndex, agent: this.codingAgent, runtimeErrors }, streamCb, toolRenderer);
     ...
     */
    async generateNextPhase(currentIssues, userContext) {
     ...
     */
    async implementPhase(phase, currentIssues, userContext, streamChunks = true, postPhaseFixing = true) {
        const issues = IssueReport.from(currentIssues);
        ...
        const result = await this.operations.implementPhase.execute({
            ...
            fileGeneratingCallback: (filePath, filePurpose) => {
                this.broadcast(WebSocketMessageResponses.FILE_GENERATING, {
            ...
            fileChunkGeneratedCallback: streamChunks ? (filePath, chunk, format) => {
        ...
        const finalFiles = await Promise.allSettled(result.fixedFilePromises).then((results) => {
    ...
    async getModelConfigsInfo() {
        ...
        try {
            ...
            for (const [actionKey, mergedConfig] of Object.entries(userConfigsRecord)) {
                ...
                const defaultConfig = AGENT_CONFIG[actionKey];
                if (defaultConfig) {
    ...
    getFileGenerated(filePath) {
        return this.fileManager.getGeneratedFile(filePath) || null;
    }
    getWebSockets() {
        return this.ctx.getWebSockets();
    }
    async fetchRuntimeErrors(clear = true) {
     ...
     */
    async runStaticAnalysisCode(files) {
        try {
     ...
     */
    async applyDeterministicCodeFixes() {
        try {
            // Get static analysis and do deterministic fixes
            const staticAnalysis = await this.runStaticAnalysisCode();
            ...
            const fixResult = fixProjectIssues(allFiles.map(file => ({
    ...
    async fetchAllIssues(resetIssues = false) {
    ...
    async updateBlueprint(patch) {
        ...
        const filtered = {};
        for (const k of keys) {
            if (allowed.has(k) && typeof patch[k] !== 'undefined') {
                filtered[k] = patch[k];
            }
        }
        if (typeof filtered.projectName === 'string' && filtered.projectName) {
            await this.updateProjectName(filtered.projectName);
            delete filtered.projectName;
        }
        const updated = { ...this.state.blueprint, ...filtered };
        this.setState({
            ...this.state,
            blueprint: updated
        });
    ...
    async execCommands(commands, shouldSave, timeout) {
        const { sandboxInstanceId } = this.state;
        if (!sandboxInstanceId) {
     ...
     */
    async regenerateFile(file, issues, retryIndex = 0) {
        this.broadcast(WebSocketMessageResponses.FILE_REGENERATING, {
            message: `Regenerating file: ${file.filePath}`,
            filePath: file.filePath,
            original_issues: issues,
        });
        const result = await this.operations.regenerateFile.execute({ file, issues, retryIndex }, this.getOperationOptions());
    ...
    async regenerateFileByPath(path, issues) {
        const { sandboxInstanceId } = this.state;
        if (!sandboxInstanceId) {
    ...
    async generateFiles(phaseName, phaseDescription, requirements, files) {
        ...
        const result = await this.implementPhase(phase, {
            runtimeErrors: [],
            staticAnalysis: {
                success: true,
                lint: { issues: [] },
        ...
        }, { suggestions: requirements }, true, // streamChunks
        ...
        return {
            files: result.files.map(f => ({
                path: f.filePath,
                purpose: f.filePurpose || '',
                diff: f.lastDiff || '' // FileState has lastDiff
            }))
        };
    }
    async deployToSandbox(files = [], redeploy = false, commitMessage, clearLogs = false) {
     ...
     */
    setGitHubToken(token, username, ttl = 3600000) {
    ...
    broadcast(msg, data) {
        ...
        broadcastToConnections(this, msg, data || {});
    }
    ...
    async saveExecutedCommands(commands) {
        ...
        const hasDependencyCommands = commands.some(cmd => cmd.includes('install') ||
            cmd.includes(' add ') ||
            cmd.includes('remove') ||
     ...
     */
    async executeCommands(commands, shouldRetry = true, chunkSize = 5) {
        ...
        for (const chunk of commandChunks) {
            ...
            while (currentChunk.length > 0 && retryCount < maxRetries) {
                try {
                    ...
                    const failedInstallCommands = failedCommands.filter(cmd => cmd.startsWith("bun") || cmd.startsWith("npm") || cmd.includes("install"));
                    if (failedInstallCommands.length > 0 && retryCount < maxRetries) {
                        // Use AI to suggest alternative commands
                        const newCommands = await this.getProjectSetupAssistant().generateSetupCommands(`The following install commands failed: ${JSON.stringify(failures, null, 2)}. Please suggest alternative commands.`);
     ...
     */
    async syncPackageJsonFromSandbox() {
        try {
    ...
    async getLogs(_reset, durationSeconds) {
        if (!this.state.sandboxInstanceId) {
    ...
    async pushToGitHub(options) {
        try {
            ...
            const appService = new AppService(this.env);
            const updateResult = await appService.updateGitHubRepository(agentId || '', options.repositoryHtmlUrl || '', options.isPrivate ? 'private' : 'public');
     ...
     */
    async handleUserInput(userMessage, images) {
        try {
            ...
            const conversationalResponse = await this.operations.processUserMessage.execute({
                ...
                conversationResponseCallback: (message, conversationId, isStreaming, tool) => {
     ...
     */
    async captureScreenshot(url, viewport = { width: 1280, height: 720 }) {
