...
export class StateMigration {
    static migrateIfNeeded(state, logger) {
        ...
        if (hasTemplateDetails) {
            ...
            migratedTemplateName = templateDetails.name;
            needsMigration = true;
        ...
        if (!state.projectName) {
            ...
            migratedProjectName = generateProjectName(state.blueprint?.projectName || migratedTemplateName || state.query, generateNanoId(), 20);
            needsMigration = true;
        ...
        if (needsMigration) {
            ...
            if (hasTemplateDetails) {
                delete newState.templateDetails;
            }
            return newState;
        }
        return null;
