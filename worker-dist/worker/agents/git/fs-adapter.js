...
export class SqliteFS {
    sql; // Assigned in constructor
    promises; // Set in init(), required by isomorphic-git
    constructor(sql) {
        this.sql = sql;
    }
    /**
     ...
     */
    getStorageStats() {
        const objects = this.sql `SELECT path, data, is_dir FROM git_objects WHERE is_dir = 0`;
    ...
    async readFile(path, options) {
        ...
        const result = this.sql `SELECT data, is_dir FROM git_objects WHERE path = ${normalized}`;
        if (!result[0]) {
    ...
    async writeFile(path, data) {
        ...
        const existing = this.sql `SELECT is_dir FROM git_objects WHERE path = ${normalized}`;
    ...
    async unlink(path) {
        ...
        if (existing[0].is_dir === 1) {
            const error = new Error(`EPERM: operation not permitted, unlink '${path}'`);
            error.code = 'EPERM';
            error.errno = -1;
            error.path = path;
            throw error;
    ...
    async readdir(path) {
        ...
        const dirCheck = this.sql `SELECT is_dir FROM git_objects WHERE path = ${normalized}`;
        ...
        const rows = this.sql `SELECT path FROM git_objects WHERE parent_path = ${normalized}`;
    ...
    async mkdir(path, _options) {
        ...
        if (!isDirectChildOfRoot) {
            ...
            const parent = this.sql `SELECT is_dir FROM git_objects WHERE path = ${parentPath}`;
    ...
    async rmdir(path) {
        ...
        const children = this.sql `SELECT path FROM git_objects WHERE path LIKE ${normalized + '/%'} LIMIT 1`;
    ...
    async stat(path) {
        const normalized = path.replace(/^\/+/, '');
        const result = this.sql `SELECT data, mtime, is_dir FROM git_objects WHERE path = ${normalized}`;
        if (!result[0]) {
    ...
    async readlink(path) {
        return (await this.readFile(path, { encoding: 'utf8' }));
    }
    /**
    ...
    exportGitObjects() {
        ...
        for (const obj of objects) {
            if (obj.is_dir === 1)
                continue; // Skip directories, only export files
