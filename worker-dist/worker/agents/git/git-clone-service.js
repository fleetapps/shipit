...
export class GitCloneService {
     ...
     */
    static async buildRepository(options) {
        ...
        try {
            ...
            for (const commitInfo of commitsOldestFirst) {
                ...
                const agentFiles = [];
                const walkTree = async (treeOid, basePath = '') => {
     ...
     */
    static async handleInfoRefs(fs) {
        try {
            ...
            try {
                ...
                branches = headsDir.filter((name) => !name.startsWith('.'));
     ...
     */
    static async handleUploadPack(fs) {
        try {
            ...
            for (const commit of commits) {
                ...
                try {
                    const collectTreeObjects = async (treeOid) => {
     ...
     */
    static wrapInSideband(packfile) {
