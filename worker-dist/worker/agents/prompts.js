...
import z from 'zod';
import { BlueprintSchemaLite, PhaseConceptLiteSchema, PhaseConceptSchema } from "./schemas";
...
export const PROMPT_UTILS = {
    ...
    serializeTreeNodes(node) {
        // The output starts with the root node's name.
        const outputParts = [node.path.split('/').pop() || node.path];
        function processChildren(children, prefix) {
...
and provide a preview url for the application.
    ...
    serializeStaticAnalysis(staticAnalysis) {
...
${typecheckOutput}`;
    ...
    serializeFiles(files, serializerType) {
        // Use scof format
...
export function generalSystemPromptBuilder(prompt, params) {
...
export function issuesPromptFormatter(issues) {
...
export const USER_PROMPT_FORMATTER = {
    PROJECT_CONTEXT: (phases, files, fileTree, commandsHistory, serializerType = CodeSerializerType.SIMPLE, recentPhasesCount = 1) => {
...
const SAAS_LANDING_INSTRUCTIONS = (style) => `
...
`;
export const getUsecaseSpecificInstructions = (selectedTemplate) => {
