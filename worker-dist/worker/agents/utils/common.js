export function extractCommands(rawOutput, onlyInstallCommands = false) {
    const commands = [];
    // Helper function to check if command is an install command
    const isInstallCommand = (command) => {
 ...
 */
export function isValidBootstrapCommand(command) {
 ...
 */
export function isBootstrapRuntimeCommand(command) {
 ...
 */
export function getPackageOperationKey(command) {
 ...
 */
export function validateAndCleanBootstrapCommands(commands, maxCommands = MAX_BOOTSTRAP_COMMANDS) {
    const seen = new Map();
