import { infer, InferError, AbortError } from './core';
import { createAssistantMessage, createUserMessage } from './common';
...
stream, tools, reasoning_effort, schema, agentActionName, format, modelName, modelConfig, context }) {
    ...
    const backoffMs = (attempt) => Math.min(500 * Math.pow(2, attempt), 10000);
 ...
 */
export function createFileEnhancementRequestMessage(filePath, fileContents) {
 ...
 */
export function createFileGenerationResponseMessage(filePath, fileContents, explanation, nextFile) {
