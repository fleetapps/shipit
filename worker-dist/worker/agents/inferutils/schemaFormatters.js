...
export function formatSchemaAsMarkdown(schema, options = {}) {
 ...
 */
function formatZodSchemaAsMarkdownFields(schema, headingPrefix, dataObject, debug = false, mode = dataObject ? 'data' : 'template') {
...
function formatZodTypeAsMarkdown(key, field, headingPrefix = '##', value, mode = 'template') {
    let optionalMarker = '';
    let baseField = field;
    let isOptional = false;
    ...
    if (!hasValue) {
        ...
        if (typeof value === 'object') {
            ...
            return `${heading}${description}\n\n${formatZodSchemaAsMarkdownFields(baseField, headingPrefix + '#', value, false, mode)}\n`;
        }
        else {
            ...
            value.forEach((item, index) => {
                ...
                if (itemSchema instanceof z.ZodObject) {
                    if (item && typeof item === 'object') {
                        ...
                        itemsMarkdown += `${itemHeading}\n\n${formatZodSchemaAsMarkdownFields(itemSchema, itemHeadingPrefix + '#', item, false, mode)}\n`;
                    }
                    else {
                        if (mode === 'template') {
...
function extractPrimitiveValueFromNodes(nodes, debugInfo) {
    ...
    const relevantNodes = nodes.filter(node => node.type === 'paragraph' || node.type === 'text' || node.type === 'inlineCode'
    ...
    const tempRoot = { type: 'root', children: relevantNodes };
...
function buildSectionTree(root) {
    const top = { heading: null, nodes: [], children: [] };
    const stack = [top];
    for (const node of root.children) {
        if (isHeading(node)) { // Use type guard
            ...
            const sec = { heading: headingNode, nodes: [], children: [] };
...
export function parseMarkdownContent(markdownInput, schema, options = {}) {
 ...
 */
function preprocessText(text, useStemming = true) {
    if (!text)
        return [];
    return text
        .split(' ')
...
path, debug, rootSchema // Keep a reference to the top-level schema if needed
    ...
    if (baseSchema instanceof z.ZodArray) {
        ...
        const directList = section.nodes.find(isList);
        if (directList && !(itemSchema instanceof z.ZodObject)) { // Only use for primitive arrays
            ...
            directList.children.forEach((listItem, i) => {
                if (isListItem(listItem)) {
                    ...
                    const itemSection = { heading: null, nodes: listItem.children, children: [] };
                    if (debug)
        ...
        if (slices.length > 0) {
            ...
            return slices.map((sliceNodes, i) => {
                const itemPath = [...path, String(i)];
                const itemSection = { heading: null, nodes: sliceNodes, children: [] };
                if (debug)
        ...
        if (!(itemSchema instanceof z.ZodObject)) {
            ...
            const contentString = extractPrimitiveValueFromNodes(section.nodes, { path: currentPath, logger: logger, debug });
            if (!contentString) {
                if (debug)
            ...
            if (looksLikeList) {
                ...
                return lines.map((ln, i) => {
                    ...
                    return convertToPrimitive(itemContent, itemSchema, { path: itemPath, logger: logger, debug });
                });
            }
                ...
                return lines.map((ln, i) => {
                    const itemPath = [...path, String(i)];
                    return convertToPrimitive(ln, itemSchema, { path: itemPath, logger: logger, debug });
                });
            }
        }
    ...
    if (debug) {
        ...
        logger.debug(`[Map Path: ${currentPath}] Nodes for primitive extraction:`, section.nodes.map(n => ({ type: n.type, value: n.value?.substring(0, 30) ?? mdastToString(n).substring(0, 30) + '...' })));
    }
    // Extract the raw string value from the section's nodes
    const extractedValue = extractPrimitiveValueFromNodes(section.nodes, { path: currentPath, logger: logger, debug });
    if (debug)
    ...
    const convertedValue = convertToPrimitive(extractedValue, schema, // Pass the original schema with wrappers
    { path: path, logger: logger, debug });
    if (debug)
...
export function generateTemplateForSchema(schema, schemaFormat, options) {
...
export function parseContentForSchema(content, schemaFormat, schema, options) {
