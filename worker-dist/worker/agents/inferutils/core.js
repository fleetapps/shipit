...
function assembleToolCalls(byIndex, byId) {
    if (byIndex.size > 0) {
        return Array.from(byIndex.values())
            ...
            .map((e) => ({ id: e.id, type: 'function', function: { name: e.function.name, arguments: e.function.arguments } }));
    }
...
export async function buildGatewayUrl(env, providerOverride) {
...
async function getApiKey(provider, env, _userId) {
    ...
    const envKey = `${providerKeyString}_API_KEY`;
    let apiKey = env[envKey];
...
export async function getConfigurationForModel(model, env, userId) {
    let providerForcedOverride;
...
export function serializeCallChain(context, finalResponse) {
...
export class InferError extends Error {
    response;
    toolCallContext;
    constructor(message, response, toolCallContext) {
        super(message);
...
export class AbortError extends InferError {
    constructor(response, toolCallContext) {
 ...
 */
async function executeToolCalls(openAiToolCalls, originalDefinitions) {
 ...
 */
export async function infer({ env, metadata, messages, schema, schemaName, actionKey, format, formatOptions, maxTokens, modelName, stream, tools, reasoning_effort, temperature, abortSignal, }, toolCallContext) {
