export function createDeepDebuggerTool(agent, logger, toolRenderer, streamCb) {
    ...
    return {
        ...
        function: {
            name: 'deep_debug',
            description: 'Autonomous debugging assistant that investigates errors, reads files, and applies fixes. CANNOT run during code generation - will return GENERATION_IN_PROGRESS error if generation is active. LIMITED TO ONE CALL PER CONVERSATION TURN.',
            parameters: {
                type: 'object',
                properties: {
                    issue: { type: 'string' },
        ...
        implementation: async ({ issue, focus_paths }) => {
