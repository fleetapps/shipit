 ...
 */
export class DeploymentManager extends BaseAgentService {
    maxCommandsHistory;
    healthCheckInterval = null;
    currentDeploymentPromise = null;
    cachedSandboxClient = null;
    constructor(options, maxCommandsHistory) {
        super(options);
     ...
     */
    async executeSetupCommands(sandboxInstanceId, timeoutMs = 60000, onAfterCommands) {
        ...
        const { validCommands, invalidCommands } = validateAndCleanBootstrapCommands(commandsHistory, this.maxCommandsHistory);
        ...
        await this.withTimeout(client.executeCommands(sandboxInstanceId, validCommands), timeoutMs, 'Command execution timed out');
     ...
     */
    startHealthCheckInterval(instanceId) {
        const logger = this.getLog();
     ...
     */
    async deployToSandbox(files = [], redeploy = false, commitMessage, clearLogs = false, callbacks) {
        const logger = this.getLog();
        ...
        if (this.currentDeploymentPromise) {
            ...
            return await this.withTimeout(this.currentDeploymentPromise, MASTER_DEPLOYMENT_TIMEOUT_MS, 'Deployment failed after 5 minutes').catch(() => null); // Convert timeout to null like first caller
        }
        ...
        this.currentDeploymentPromise = this.executeDeploymentWithRetry(files, redeploy, commitMessage, clearLogs, callbacks);
        try {
            // Master timeout: 5 minutes total
            // This doesn't break the underlying operation - it just stops waiting
            const result = await this.withTimeout(this.currentDeploymentPromise, MASTER_DEPLOYMENT_TIMEOUT_MS, 'Deployment failed after 5 minutes of retries'
     ...
     */
    async executeDeploymentWithRetry(files, redeploy, commitMessage, clearLogs, callbacks) {
        const logger = this.getLog();
        let attempt = 0;
        ...
        while (true) {
            ...
            try {
                ...
                const result = await this.withTimeout(deployPromise, PER_ATTEMPT_TIMEOUT_MS, `Deployment attempt ${attempt} timed out`
    ...
    async createNewInstance() {
        ...
        if (state.templateName?.includes('agents')) {
            ...
            if (typeof secret === 'string' && secret.trim().length > 0) {
                localEnvVars = {
                    ...
                    "CF_AI_API_KEY": await generateAppProxyToken(state.inferenceContext.agentId, state.inferenceContext.userId, this.env)
                };
            }
        }
        // Create instance
        const client = this.getClient();
        const logger = this.getLog();
        const createResponse = await client.createInstance(templateName, `v1-${projectName}`, undefined, localEnvVars);
     ...
     * Determine which files to deploy
     */
    getFilesToDeploy(requestedFiles, redeployed) {
        const state = this.getState();
     ...
     */
    async deployToCloudflare(callbacks) {
        const state = this.getState();
        const logger = this.getLog();
        const client = this.getClient();
        ...
        const deploymentResult = await client.deployToCloudflareWorkers(state.sandboxInstanceId);
