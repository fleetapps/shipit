import { FileOutputType, Blueprint, FileConceptType } from "../../../agents/schemas";
import { BaseSandboxService } from "../../../services/sandbox/BaseSandboxService";
import { ExecuteCommandsResponse, PreviewType, StaticAnalysisResponse, RuntimeError } from "../../../services/sandbox/sandboxTypes";
...
import { GitVersionControl } from "../../../agents/git/git";
export declare abstract class ICodingAgent {
    ...
    abstract deployToCloudflare(): Promise<{
    ...
    abstract readFiles(paths: string[]): Promise<{
        files: {
            path: string;
            content: string;
        }[];
    ...
    abstract regenerateFileByPath(path: string, issues: string[]): Promise<{
        path: string;
        diff: string;
    }>;
    abstract generateFiles(phaseName: string, phaseDescription: string, requirements: string[], files: FileConceptType[]): Promise<{
        files: Array<{
            path: string;
            purpose: string;
            diff: string;
    ...
    abstract executeDeepDebug(issue: string, toolRenderer: RenderToolCall, streamCb: (chunk: string) => void, focusPaths?: string[]): Promise<DeepDebugResult>;
}
