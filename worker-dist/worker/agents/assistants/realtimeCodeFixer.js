...
import { MatchingStrategy } from "../output-formats/diff-formats/search-replace";
...
[your intended replacement]
>>>>>>> REPLACE`;
const userPromptFormatter = (user_prompt, query, file, previousFiles, currentPhase, issues) => {
    const variables = {
        query,
...
const diffPromptFormatter = (currentContent, failedBlocks, failedBlocksCount, successfulBlocksCount) => {
...
export class RealtimeCodeFixer extends Assistant {
    logger = createObjectLogger(this, 'RealtimeCodeFixer');
    lightMode;
    altPassModelOverride;
    userPrompt;
    systemPrompt;
    agentActionNameOverride;
    constructor(env, inferenceContext, lightMode = false, altPassModelOverride, // = AIModels.GEMINI_2_5_FLASH,
    agentActionNameOverride, systemPrompt = SYSTEM_PROMPT, userPrompt = USER_PROMPT) {
    ...
    async run(generatedFile, context, currentPhase, issues = [], passes = DEFAULT_PASSES) {
        try {
...
`);
     ...
     */
    async applyDiffSafely(originalContent, originalDiff, maxRetries = MAX_RETRIES) {
        ...
        try {
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                ...
                if (searchBlocks !== replaceBlocks) {
                    ...
                    const correctedDiff = await this.getLLMCorrectedDiff(currentContent, [], [`Mismatched search and replace blocks in current diff: ${searchBlocks} search blocks and ${replaceBlocks} replace blocks. Current diff: \n${currentDiff}`], 0);
                    if (!correctedDiff) {
...
-------------------------`);
                ...
                const correctedDiff = await this.getLLMCorrectedDiff(currentContent, failedBlocks, result.results.errors, blocksApplied);
                if (!correctedDiff) {
                    this.logger.warn(`❌ Failed to get LLM correction on attempt ${attempt + 1}`);
     ...
     */
    async getLLMCorrectedDiff(currentContent, failedBlocks, allErrors, successfullyAppliedCount) {
        try {
            // Format the failed blocks in the expected format for the new prompt
            const failedBlocksText = failedBlocks.map((block) => `## SearchReplaceNoExactMatch: This SEARCH block failed to exactly match lines in the file
<<<<<<< SEARCH
${block.search}
=======
${block.replace}
...
`).join('\n\n');
            ...
            const diffFixerPrompt = diffPromptFormatter(currentContent, failedBlocksText + allErrorsText, failedBlocks.length, successfullyAppliedCount);
...
export function IsRealtimeCodeFixerEnabled(inferenceContext) {
