...
const USER_PROMPT = (issue, fileSummaries, templateInfo, runtimeErrors, previousTranscript) => `## Debugging Task
...
Begin.`;
function summarizeFiles(files, max = 120) {
    const compact = files
        .slice(0, max)
        .map((f) => {
...
export class DeepCodeDebugger extends Assistant {
    ...
    constructor(env, inferenceContext, modelConfigOverride) {
    ...
    detectRepetition(toolName, args) {
        ...
        this.loopDetection.recentCalls = this.loopDetection.recentCalls.filter((call) => now - call.timestamp < 600000);
        // Count how many times this exact call was made recently
        const matchingCalls = this.loopDetection.recentCalls.filter((call) => call.toolName === toolName && call.args === argsStr);
        // Record this call
...
If you're genuinely stuck after trying 3 different approaches, honestly report: "TASK_STUCK: [reason]"`;
    ...
    async run(inputs, session, streamCb, toolRenderer) {
        ...
        const user = createUserMessage(USER_PROMPT(inputs.issue, fileSummaries, templateInfo, session.runtimeErrors ? PROMPT_UTILS.serializeErrors(session.runtimeErrors) : undefined, inputs.previousTranscript));
        const messages = this.save([system, user]);
