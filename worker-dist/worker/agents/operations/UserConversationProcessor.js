import { createAssistantMessage, createUserMessage, createMultiModalUserMessage, mapImagesInMultiModalMessage } from "../inferutils/common";
...
export function buildToolCallRenderer(callback, conversationId) {
    return (args) => {
...
{{userMessage}}
`;
function buildUserMessageWithContext(userMessage, errors, projectUpdates, forInference) {
...
async function prepareMessagesForInference(env, messages) {
...
export class UserConversationProcessor extends AgentOperation {
    ...
    async execute(inputs, options) {
        ...
        try {
            ...
            const tools = buildTools(agent, logger, toolCallRenderer, (chunk) => inputs.conversationResponseCallback(chunk, aiConversationId, true)).map(td => ({
                ...td,
                onStart: (args) => toolCallRenderer({ name: td.function.name, status: 'start', args }),
                onComplete: (args, result) => toolCallRenderer({
            ...
            const userMessageForHistory = images && images.length > 0
                ? createMultiModalUserMessage(userPromptForHistory, images.map(img => img.r2Key), 'high')
            ...
            if (result.toolCallContext?.messages && result.toolCallContext.messages.length > 0) {
                messages.push(...result.toolCallContext.messages
            ...
            if (compactHistory.length !== runningHistory.length) {
                ...
                const originalById = new Map(originalRunning.map(m => [m.conversationId, m]));
     ...
     */
    findTurnBoundary(messages, preserveCount) {
     ...
     */
    async generateConversationSummary(messages, env, options, logger) {
        try {
            // Prepare summarization instruction
            const summarizationInstruction = createUserMessage(`Please provide a comprehensive summary of the entire conversation above. Your summary should:
...
Provide the summary now:`);
     ...
     */
    async compactifyContext(runningHistory, env, options, toolCallRenderer, logger) {
        try {
            ...
            const splitIndex = this.findTurnBoundary(runningHistory, COMPACTIFICATION_CONFIG.PRESERVE_RECENT_MESSAGES);
            ...
            const summary = await this.generateConversationSummary(messagesToSummarize, env, options, logger);
    ...
    processProjectUpdates(updateType, _data, logger) {
        try {
