...
const formatUserSuggestions = (suggestions) => {
...
const specialPhasePromptOverrides = {
    "Finalization and Review": LAST_PHASE_PROMPT,
};
const userPromptFormatter = (phaseConcept, issues, userSuggestions) => {
            ...
            stream: {
                ...
                onChunk: (chunk) => {
                    ...
                    (filePath) => {
                        ...
                        completedFile.fileContents = FileProcessing.processGeneratedFileContents(completedFile, originalContents, logger);
                        const generatedFile = {
                            ...completedFile,
                            filePurpose: FileProcessing.findFilePurpose(filePath, phase, context.allFiles.reduce((acc, f) => ({ ...acc, [f.filePath]: f }), {}))
                        };
                        if (shouldEnableRealtimeCodeFixer && generatedFile.fileContents.split('\n').length > 50) {
                            ...
                            const fixPromise = realtimeCodeFixer.run(generatedFile, {
