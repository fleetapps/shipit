 ...
 */
function breakIntoSubHunks(hunk, maxContextLines = 3) {
    const subHunks = [];
    ...
    for (const run of changeRuns) {
        ...
        const contextBefore = hunk.slice(Math.max(0, run.start - maxContextLines), run.start).filter(line => line.startsWith(' ')); // Only context lines
        const contextAfter = hunk.slice(run.end + 1, Math.min(hunk.length, run.end + 1 + maxContextLines)).filter(line => line.startsWith(' ')); // Only context lines
...
function tryWithWhitespaceNormalization(content, before, after) {
    ...
    const normalizeLineWhitespace = (line) => {
...
function tryContextReduction(content, hunk) {
    ...
    const lastChangeIndex = hunk.length -
        1 -
        [...hunk]
            .reverse()
 ...
 */
export function applyDiff(originalContent, diffContent, options = {}) {
    const telemetry = {
        strategiesAttempted: [],
        processingTimeMs: 0,
