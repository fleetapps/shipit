...
function extractIndentationPattern(text) {
    ...
    const pattern = indentedLines.map(({ indent, content }) => content.trim() ? `${indent.length}:${content.trim()}` : '').join('\n');
...
function findFuzzyMatch(content, searchText, threshold = 0.8) {
 ...
 */
export function createSearchReplaceDiff(beforeContent, afterContent, options = {}) {
 ...
 */
export function validateDiff(content, diffContent) {
    const { blocks, errors: parseErrors } = parseSearchReplaceDiff(diffContent);
