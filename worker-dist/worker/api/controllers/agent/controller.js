...
export class CodingAgentController extends BaseController {
     ...
     */
    static async startCodeGeneration(request, env, _, context) {
        try {
            ...
            agentPromise.then(async (_state) => {
     ...
     */
    static async handleWebSocketConnection(request, env, _, context) {
        try {
            ...
            if (!user) {
                ...
                if (!canAccess) {
                    ...
                    return CodingAgentController.createErrorResponse('Anonymous access expired. Please log in to continue, or reconnect within 1 hour of creation.', 401);
                }
     ...
     */
    static async verifyAnonymousAccess(agentId, env) {
        try {
            const appService = new AppService(env);
     ...
     */
    static async connectToExistingAgent(request, env, _, context) {
        try {
            ...
            try {
                ...
                if (!agentInstance || !(await agentInstance.isInitialized())) {
                    return CodingAgentController.createErrorResponse('Agent instance not found or not initialized', 404);
                }
                ...
                return CodingAgentController.createErrorResponse(`Agent instance not found or unavailable: ${error instanceof Error ? error.message : String(error)}`, 404);
            }
        }
        catch (error) {
            this.logger.error('Error connecting to existing agent', error);
            return CodingAgentController.handleError(error, 'connect to existing agent');
        }
    }
    static async deployPreview(_request, env, _, context) {
        try {
            const agentId = context.pathParams.agentId;
            if (!agentId) {
                return CodingAgentController.createErrorResponse('Missing agent ID parameter', 400);
            ...
            try {
                ...
                if (!preview) {
                    return CodingAgentController.createErrorResponse('Failed to deploy preview', 500);
                }
            ...
            const appError = CodingAgentController.handleError(error, 'deploy preview');
            return appError;
        }
    }
    /**
     ...
     */
    static async triggerCodeGeneration(request, env, _, context) {
        try {
            const agentId = context.pathParams.agentId;
            if (!agentId) {
                return CodingAgentController.createErrorResponse('Missing agent ID parameter', 400);
            ...
            try {
                ...
                if (!isInitialized) {
                    ...
                    if (user?.id) {
                        ...
                        const emptyModelConfigs = new Map();
                        inferenceContext = {
                            userModelConfigs: Object.fromEntries(emptyModelConfigs),
                            agentId: agentId,
                    ...
                    const url = new URL(request.url);
                    const hostname = url.hostname === 'localhost' ? `localhost:${url.port}` : getPreviewDomain(env);
                    const { templateDetails, selection } = await getTemplateForQuery(env, inferenceContext, app.originalPrompt, undefined, // No images available in fallback
                    this.logger);
            ...
            return CodingAgentController.handleError(error, 'trigger code generation');
        }
    }
}
