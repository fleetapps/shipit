...
export class AuthController extends BaseController {
     ...
     */
    static async register(request, env, _ctx, _routeContext) {
        try {
            // Check if OAuth providers are configured - if yes, block email/password registration
            if (AuthController.hasOAuthProviders(env)) {
                return AuthController.createErrorResponse('Email/password registration is not available when OAuth providers are configured. Please use OAuth login instead.', 403);
            }
            ...
            if (env.ALLOWED_EMAIL && validatedData.email !== env.ALLOWED_EMAIL) {
                return AuthController.createErrorResponse('Email Whitelisting is enabled. Please use the allowed email to register.', 403);
            }
            const authService = new AuthService(env);
            const result = await authService.register(validatedData, request);
            const response = AuthController.createSuccessResponse(formatAuthResponse(result.user, result.sessionId, result.expiresAt));
     ...
     * POST /api/auth/login
     */
    static async login(request, env, _ctx, _routeContext) {
        try {
            // Check if OAuth providers are configured - if yes, block email/password login
            if (AuthController.hasOAuthProviders(env)) {
                return AuthController.createErrorResponse('Email/password login is not available when OAuth providers are configured. Please use OAuth login instead.', 403);
            }
            const bodyResult = await AuthController.parseJsonBody(request);
            if (!bodyResult.success) {
                return bodyResult.response;
            ...
            if (env.ALLOWED_EMAIL && validatedData.email !== env.ALLOWED_EMAIL) {
                return AuthController.createErrorResponse('Email Whitelisting is enabled. Please use the allowed email to login.', 403);
            }
            const authService = new AuthService(env);
     ...
     */
    static async logout(request, env, _ctx, _routeContext) {
        try {
     ...
     */
    static async getProfile(_request, _env, _ctx, routeContext) {
        try {
     ...
     */
    static async updateProfile(request, env, _ctx, routeContext) {
        try {
     ...
     */
    static async initiateOAuth(request, env, _ctx, routeContext) {
        try {
     ...
     */
    static async handleOAuthCallback(request, env, _ctx, routeContext) {
        try {
            const validatedProvider = oauthProviderSchema.parse(routeContext.pathParams.provider);
     ...
     * GET /api/auth/check
     */
    static async checkAuth(request, env, _ctx, _routeContext) {
        try {
     ...
     */
    static async getActiveSessions(_request, env, _ctx, routeContext) {
        try {
            const user = routeContext.user;
            if (!user) {
                return AuthController.createErrorResponse('Unauthorized', 401);
     ...
     */
    static async revokeSession(_request, env, _ctx, routeContext) {
        try {
            const user = routeContext.user;
            if (!user) {
                return AuthController.createErrorResponse('Unauthorized', 401);
     ...
     */
    static async getApiKeys(_request, env, _ctx, routeContext) {
        try {
            const user = routeContext.user;
            if (!user) {
                return AuthController.createErrorResponse('Unauthorized', 401);
     ...
     */
    static async createApiKey(request, env, _ctx, routeContext) {
        try {
            const user = routeContext.user;
            if (!user) {
                return AuthController.createErrorResponse('Unauthorized', 401);
     ...
     */
    static async revokeApiKey(_request, env, _ctx, routeContext) {
        try {
            const user = routeContext.user;
            if (!user) {
                return AuthController.createErrorResponse('Unauthorized', 401);
     ...
     */
    static async verifyEmail(request, env, _ctx, _routeContext) {
        try {
            const bodyResult = await AuthController.parseJsonBody(request);
            if (!bodyResult.success) {
                return bodyResult.response;
     ...
     */
    static async resendVerificationOtp(request, env, _ctx, _routeContext) {
        try {
            const bodyResult = await AuthController.parseJsonBody(request);
            if (!bodyResult.success) {
                return bodyResult.response;
     ...
     */
    static async getAuthProviders(request, env, _ctx, _context) {
        try {
