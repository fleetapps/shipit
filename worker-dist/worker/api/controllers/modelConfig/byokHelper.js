 ...
 */
export async function getUserProviderStatus(userId, env) {
    try {
        ...
        for (const template of byokTemplates) {
            ...
            const providerSecret = userSecrets.find((secret) => secret.secretType === template.envVarName &&
                secret.isActive);
 ...
 */
export function getByokModels(providerStatuses) {
 ...
 */
export function getPlatformEnabledProviders(env) {
 ...
 */
export function getPlatformAvailableModels(env) {
 ...
 */
export function validateModelAccessForEnvironment(model, env, userProviderStatus) {
    const provider = getProviderFromModel(model);
