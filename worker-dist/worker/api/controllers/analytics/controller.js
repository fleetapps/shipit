...
export class AnalyticsController extends BaseController {
     ...
     */
    static async getUserAnalytics(request, env, _ctx, context) {
        try {
            ...
            if (!userId) {
                return AnalyticsController.createErrorResponse('User ID is required', 400);
            }
            ...
            if (daysParam) {
                ...
                if (isNaN(days) || days < 1 || days > 365) {
                    return AnalyticsController.createErrorResponse('Days must be between 1 and 365', 400);
                }
            }
            // Get analytics data
            ...
            if (error instanceof AnalyticsError) {
                return AnalyticsController.createErrorResponse(error.message, error.statusCode);
            }
            return AnalyticsController.createErrorResponse('Failed to fetch user analytics', 500);
        }
    }
    /**
     ...
     */
    static async getAgentAnalytics(request, env, _ctx, context) {
        try {
            // Extract authenticated user from context
            const authUser = context.user;
            // Extract route parameters
            ...
            if (!agentId) {
                return AnalyticsController.createErrorResponse('Agent ID is required', 400);
            }
            ...
            return AnalyticsController.createErrorResponse('Failed to fetch agent analytics', 500);
        }
    }
}
