...
export class GitHubExporterController extends BaseController {
     ...
     */
    static async createRepositoryAndPush(options) {
    ...
    static async handleOAuthCallback(request, env, _ctx, context) {
        try {
            ...
            if (error) {
                ...
                return Response.redirect(`${new URL(request.url).origin}/settings?integration=github&status=error&reason=${encodeURIComponent(error)}`, 302);
            }
            if (!code) {
                return Response.redirect(`${new URL(request.url).origin}/settings?integration=github&status=error&reason=missing_code`, 302);
            }
            ...
            if (!parsedState || !parsedState.userId) {
                return Response.redirect(`${new URL(request.url).origin}/settings?integration=github&status=error&reason=invalid_state`, 302);
            }
            ...
            if (!tokenResult || !tokenResult.accessToken) {
                ...
                return Response.redirect(`${returnUrl}?github_export=error&reason=token_exchange_failed`, 302);
            }
            ...
            if (purpose === 'repository_export' && exportData && agentId) {
                ...
                if (!result.success) {
                    return Response.redirect(`${returnUrl}?github_export=error&reason=${encodeURIComponent(result.error)}`, 302);
                }
                this.logger.info('OAuth export completed', { userId, agentId, repositoryUrl: result.repositoryUrl });
                return Response.redirect(`${returnUrl}?github_export=success&repository_url=${encodeURIComponent(result.repositoryUrl)}`, 302);
            }
            return Response.redirect(`${returnUrl}?integration=github&status=oauth_success`, 302);
        }
        catch (error) {
    ...
    static async initiateGitHubExport(request, env, _ctx, context) {
        try {
            if (!context.user) {
                return GitHubExporterController.createErrorResponse('Authentication required', 401);
            }
            const body = await request.json();
            this.logger.info('Export initiated', { userId: context.user.id, agentId: body.agentId });
            if (!body.repositoryName) {
                return GitHubExporterController.createErrorResponse('Repository name is required', 400);
            }
            if (!body.agentId) {
                return GitHubExporterController.createErrorResponse('Instance ID is required for file pushing', 400);
            }
            ...
            const baseUrl = new URL(request.url).origin;
            const oauthProvider = GitHubExporterOAuthProvider.create(env, baseUrl);
            const authUrl = await oauthProvider.getAuthorizationUrl(Buffer.from(JSON.stringify(state)).toString('base64'));
            ...
            return GitHubExporterController.createErrorResponse('Failed to initiate GitHub export', 500);
        }
    }
    static async checkRemoteStatus(request, env, _ctx, context) {
        try {
            if (!context.user) {
                return GitHubExporterController.createErrorResponse('Authentication required', 401);
            }
            ...
            if (!body.repositoryUrl || !body.agentId) {
                return GitHubExporterController.createErrorResponse('Repository URL and agent ID are required', 400);
            }
            const agentStub = await getAgentStub(env, body.agentId);
            // Try to get cached token
            const cachedToken = await agentStub.getGitHubToken();
            if (!cachedToken) {
                return GitHubExporterController.createErrorResponse('No cached GitHub token. Please re-authenticate.', 401);
            }
            ...
            return GitHubExporterController.createErrorResponse('Failed to check remote status', 500);
        }
    }
}
