 ...
 */
export function isGitProtocolRequest(pathname) {
 ...
 */
function extractAgentHeadOid(gitObjects) {
 ...
 */
async function verifyGitAccess(request, env, appId) {
 ...
 */
async function handleInfoRefs(request, env, ctx, appId) {
    try {
        // Verify access first
 ...
 */
async function handleUploadPack(request, env, _ctx, appId) {
    try {
        // Verify access first
        const { hasAccess, appCreatedAt } = await verifyGitAccess(request, env, appId);
        if (!hasAccess) {
 ...
 */
export async function handleGitProtocolRequest(request, env, ctx) {
