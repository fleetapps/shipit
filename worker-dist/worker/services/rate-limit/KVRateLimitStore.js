...
export class KVRateLimitStore {
    ...
    static async increment(kv, key, config, incrementBy = 1) {
        const now = Date.now();
        ...
        try {
            ...
            const bucketResults = await Promise.all(allBucketKeys.map(async (bucketKey) => {
    ...
    static async getRemainingLimit(kv, key, config) {
        const now = Date.now();
        const bucketSize = (config.bucketSize ?? 10) * 1000; // ms
        const mainWindow = config.period * 1000; // ms
        const mainBuckets = this.generateBucketKeys(key, now, mainWindow, bucketSize);
        const counts = await Promise.all(mainBuckets.map(async (bucketKey) => {
            const value = await kv.get(bucketKey);
    ...
    static generateBucketKeys(key, now, windowMs, bucketSizeMs) {
        const buckets = [];
    ...
    static async incrementBucketWithRetry(kv, bucketKey, ttlSeconds, maxRetries = 3, incrementBy = 1) {
