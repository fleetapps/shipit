...
import { createUnfixableIssue, createExternalModuleError, handleFixerError, createFixerLogMessages, createSourceFileParseError, createMissingImportError } from '../utils/helpers';
 ...
 */
export function fixIncorrectNamedImport(context, issues) {
    ...
    for (const [filePath, fileIssues] of issuesByFile) {
        try {
            ...
            const replacementsByImport = new Map();
            ...
            for (const issue of fileIssues) {
                unfixableIssues.push(handleFixerError(issue, error, 'TS2724Fixer'));
            }
        }
    }
 ...
 */
export function parseTS2724ErrorMessage(errorMessage) {
 ...
 */
export function replaceNamedImport(ast, moduleSpecifier, oldImportName, newImportName) {
            ...
            node.specifiers = node.specifiers.map(specifier => {
                if (t.isImportSpecifier(specifier)) {
                    ...
                    if (importedName === oldImportName) {
                        ...
                        const newSpecifier = t.importSpecifier(specifier.local, // Keep the same local name
 ...
 */
export function applyMultipleNamedImportReplacements(ast, replacementsByImport) {
