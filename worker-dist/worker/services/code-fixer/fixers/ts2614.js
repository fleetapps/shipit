 ...
 */
export function fixImportExportTypeMismatch(context, issues) {
    ...
    for (const issue of issues) {
        ...
        try {
            ...
            unfixableIssues.push(handleFixerError(issue, error, 'TS2614Fixer'));
        }
    }
...
function analyzeMismatch(importInfo, targetExports) {
    ...
    if (importInfo.namedImports.length > 0 && !importInfo.defaultImport && targetExports.defaultExport) {
        ...
        const matchingNamedImport = importInfo.namedImports.find(name => name === targetExports.defaultExport ||
    ...
    if (importInfo.defaultImport && !targetExports.defaultExport && targetExports.namedExports.length > 0) {
        ...
        const matchingNamedExport = targetExports.namedExports.find(name => name === importInfo.defaultImport ||
    ...
    if (importInfo.namedImports.length > 0) {
        ...
        const needsDefaultConversion = invalidNamedImports.filter(name => targetExports.defaultExport && (name === targetExports.defaultExport ||
            name.toLowerCase() === targetExports.defaultExport?.toLowerCase()));
                ...
                switch (mismatchAnalysis.type) {
                    case 'named-to-default':
                        ...
                        if (mismatchAnalysis.targetName) {
                            ...
                            const orig = path.node.specifiers.find((s) => t.isImportSpecifier(s) &&
                    ...
                    case 'complex-partial':
                        ...
                        if (mismatchAnalysis.additionalData) {
                            ...
                            if (defaultConversions.length > 0) {
                                ...
                                const defaultSpec = specifiers.find((s) => t.isImportSpecifier(s) &&
                                    t.isIdentifier(s.imported) &&
                            ...
                            if (mismatchAnalysis.sourceNames) {
                                for (const validName of mismatchAnalysis.sourceNames) {
                                    const orig = specifiers.find((s) => t.isImportSpecifier(s) &&
                                        t.isIdentifier(s.imported) &&
                            ...
                            const invalidSpec = specifiers.find((s) => t.isImportSpecifier(s) &&
                                t.isIdentifier(s.imported) &&
                                s.imported.name === mismatchAnalysis.targetName);
