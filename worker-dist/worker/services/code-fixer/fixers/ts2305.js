...
import { getSourceAndTargetFiles, createUnfixableIssue, createExternalModuleError, handleFixerError, createFixerLogMessages } from '../utils/helpers';
 ...
 */
export function fixMissingExportedMember(context, issues) {
    ...
    for (const issue of issues) {
        ...
        try {
            ...
            if (!filesResult) {
                ...
                unfixableIssues.push(createUnfixableIssue(issue, 'Could not resolve source file, import location, or target file'));
                continue;
            }
            ...
            if (existingExports.namedExports.includes(missingExportName)) {
                ...
                unfixableIssues.push(createUnfixableIssue(issue, `Named export ${missingExportName} already exists in target file`));
                continue;
            }
            ...
            if (existingExports.defaultExport && existingExports.defaultExport === missingExportName) {
                ...
                unfixableIssues.push(createUnfixableIssue(issue, `Default export ${missingExportName} already exists in target file`));
                continue;
            }
            ...
            const modifiedTargetAST = addExportToFile(targetAST, missingExportName, exportUsage ? {
                ...
                parameters: exportUsage.parameters?.map(p => String(p)) || undefined
            } : undefined);
            ...
            unfixableIssues.push(handleFixerError(issue, error, 'TS2305Fixer'));
        }
    }
...
function createComponentExport(exportName, props, stubComment) {
    ...
    if (t.isObjectPattern(propsParam)) {
        const propsType = t.tsTypeLiteral(props.map(prop => {
    ...
    const componentBody = t.blockStatement([
        t.returnStatement(t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier('div'), [], false), t.jsxClosingElement(t.jsxIdentifier('div')), [t.jsxText(`${stubComment} - Component: ${exportName}`)], false))
    ]);
...
function createObjectExport(exportName, properties, stubComment) {
    const objectProperties = properties.map(prop => t.objectProperty(t.identifier(prop), t.arrowFunctionExpression([], t.blockStatement([
        t.returnStatement(t.nullLiteral())
    ]))));
