 ...
 */
export function analyzeImportUsageForStub(importInfo, files) {
 ...
 */
export function generateStubFileAST(importInfo, usages) {
    const statements = [];
 ...
 */
export function generateStubFileContent(importInfo, files) {
...
function generateJSXComponentExport(usage) {
    ...
    if (t.isObjectPattern(propsParam)) {
        const propsType = props.length > 0 ?
            t.tsTypeLiteral(props.map(prop => {
    ...
    const componentBody = t.blockStatement([
        t.returnStatement(t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier('div'), [], false), t.jsxClosingElement(t.jsxIdentifier('div')), [t.jsxText(`Placeholder ${componentName} component`)], false))
    ]);
...
function generateGenericExport(name, isDefault, shouldUseJSX) {
    ...
    if (shouldUseJSX) {
        ...
        const componentFunc = t.arrowFunctionExpression([], t.jsxElement(t.jsxOpeningElement(t.jsxIdentifier('div'), [], false), t.jsxClosingElement(t.jsxIdentifier('div')), [t.jsxText(`Placeholder ${name} component`)], false));
