 ...
 */
export function findImportAtLocation(ast, line) {
 ...
 */
export function getAllImports(ast) {
    const imports = [];
    traverseAST(ast, {
 ...
 */
export function getFileExports(ast) {
 ...
 * Preserves exact logic from working implementation
 */
export function analyzeImportUsage(ast, importNames) {
    const usages = [];
 ...
 * Preserves exact logic from working implementation
 */
export function analyzeNameUsage(ast, name) {
    let usage = null;
    const properties = [];
    traverseAST(ast, {
 ...
 */
export function getFileContent(filePath, files) {
 ...
 */
export function getFileAST(filePath, files) {
 ...
 */
export function updateImportPath(ast, oldPath, newPath) {
    const body = ast.program?.body ?? [];
    for (const node of body) {
 ...
 */
export function fixImportExportMismatch(ast, moduleSpecifier, exports) {
    let fixed = false;
    const changes = [];
    traverseAST(ast, {
        ImportDeclaration(path) {
                ...
                if (namespaceImport && t.isImportNamespaceSpecifier(namespaceImport)) {
                    ...
                    if (defaultImport) {
                        ...
                        if (exports.defaultExport) {
                            ...
                            const targetNamed = exports.namedExports.find(n => n === localName || n.toLowerCase() === localName.toLowerCase()) || exports.namedExports[0];
                            if (targetNamed) {
                    ...
                    for (const namedImport of namedImports) {
                        if (t.isImportSpecifier(namedImport) && t.isIdentifier(namedImport.imported)) {
                            ...
                            if (exports.namedExports.includes(namedImportName)) {
                                ...
                                const caseInsensitiveMatch = exports.namedExports.find(n => n.toLowerCase() === namedImportName.toLowerCase());
