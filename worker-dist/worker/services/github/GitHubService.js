...
export class GitHubService {
    static logger = createLogger('GitHubService');
    ...
    static async createUserRepository(options) {
        ...
        try {
            ...
            if (octokitError?.status === 422) {
                const hasNameExistsError = octokitError?.response?.data?.errors?.some((e) => e.field === 'name' && e.message?.includes('already exists'));
     ...
     */
    static async modifyReadmeForGitHub(fs, githubRepoUrl) {
        try {
     ...
     */
    static normalizeCommitMessage(message) {
        return message.trim();
    }
    /**
     * Check if commit is system-generated
     */
    static isSystemGeneratedCommit(message) {
     ...
     */
    static findLastCommonCommit(localCommits, remoteCommits) {
     ...
     */
    static async pushViaGitProtocol(fs, token, repositoryUrl) {
        try {
    ...
    static async checkRemoteStatus(options) {
        try {
            ...
            const localOnly = localCommits.filter(c => !remoteMessages.has(GitHubService.normalizeCommitMessage(c.commit.message)));
            const remoteOnly = remoteCommits.filter(c => !localMessages.has(GitHubService.normalizeCommitMessage(c.commit.message)) &&
