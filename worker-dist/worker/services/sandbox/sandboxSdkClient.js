...
import { buildDeploymentConfig, parseWranglerConfig, deployToDispatch, } from '../deployer/deploy';
...
export class SandboxSdkClient extends BaseSandboxService {
     ...
     */
    async getInstanceSession(instanceId, cwd) {
     ...
     */
    async safeSandboxExec(command, options) {
     ...
     */
    invalidateSessionCache(instanceId) {
        if (this.sessionCache.has(instanceId)) {
    ...
    async writeBinaryFileViaBase64(targetPath, data, bytesPerChunk = 16 * 1024) {
     ...
     */
    async writeFilesViaScript(files, session) {
        if (files.length === 0)
            return [];
        ...
        for (const { path, content } of files) {
            ...
            scriptLines.push(`mkdir -p "$(dirname "${path}")" && echo '${base64}' | base64 -d > "${path}" && echo "OK:${path}" || echo "FAIL:${path}"`);
        }
    ...
    async updateProjectName(instanceId, projectName) {
        try {
            ...
            try {
                ...
                const updated = { ...metadata, projectName };
    ...
    getInstanceMetadataFile(instanceId) {
    ...
    async storeInstanceMetadata(instanceId, metadata) {
        const session = await this.getDefaultSession();
    ...
    invalidateMetadataCache(instanceId) {
        this.metadataCache.delete(instanceId);
    }
    async allocateAvailablePort(excludedPorts = [3000]) {
    ...
    async downloadTemplate(templateName, downloadDir) {
    ...
    async ensureTemplateExists(templateName, downloadDir, isInstance = false) {
    ...
    async buildFileTree(instanceId) {
        try {
    ...
    async listAllInstances() {
        try {
            ...
            const metadataSections = bulkResult.stdout.split('===FILE:').filter((section) => section.trim());
     ...
     */
    async waitForServerReady(instanceId, processId, maxWaitTimeMs = 10000) {
        const startTime = Date.now();
    ...
    async startDevServer(instanceId, port) {
        try {
            ...
            const process = await session.startProcess(`VITE_LOGGER_TYPE=json PORT=${port} monitor-cli process start --instance-id ${instanceId} --port ${port} -- bun run dev`);
     ...
     */
    async provisionTemplateResources(instanceId, projectName) {
        try {
            const session = await this.getInstanceSession(instanceId);
    ...
    */
    async startCloudflaredTunnel(instanceId, port) {
        try {
     ...
     */
    async updateProjectConfiguration(instanceId, projectName) {
        try {
            const session = await this.getInstanceSession(instanceId);
    ...
    async setLocalEnvVars(instanceId, localEnvVars) {
        try {
    ...
    async setupInstance(instanceId, projectName, localEnvVars) {
        try {
    ...
    async fetchDontTouchFiles(templateName) {
        let donttouchFiles = [];
        try {
    ...
    async fetchRedactedFiles(templateName) {
        let redactedFiles = [];
        try {
                ...
                if (metadata.processId) {
                    for (let i = 0; i < 3; i++) {
                        try {
                            ...
                            const process = processes.find((p) => p.id === metadata.processId);
    ...
    // FILE OPERATIONS
    // ==========================================
    async writeFiles(instanceId, files) {
        try {
            const session = await this.getInstanceSession(instanceId);
            const results = [];
    ...
    async getFiles(templateOrInstanceId, filePaths, applyFilter = true, redactedFiles) {
        try {
    ...
    // ERROR MANAGEMENT
    // ==========================================
    async getInstanceErrors(instanceId, clear) {
        try {
            let errors = [];
    ...
    // ==========================================
    async runStaticAnalysisCode(instanceId) {
        try {
            const lintIssues = [];
            ...
            if (lintResult.status === 'fulfilled') {
                try {
                    const lintData = JSON.parse(lintResult.value.stdout);
    ...
    async deployToCloudflareWorkers(instanceId) {
        try {
            ...
            const deployConfig = buildDeploymentConfig(dispatchConfig, workerContent, accountId, apiToken, assetsManifest, config.compatibility_flags);
            ...
            if ('DISPATCH_NAMESPACE' in env) {
                ...
                }, fileContents, additionalModules, config.migrations, config.assets);
            }
            else {
     ...
     */
    async processAssetsInSandbox(_instanceId, assetsPath) {
        ...
        const filesAsArrayBuffer = new Map();
     ...
     */
    async readFileAsBase64Buffer(filePath) {
