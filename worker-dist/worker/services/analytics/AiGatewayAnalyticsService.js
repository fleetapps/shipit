...
export class AiGatewayAnalyticsService {
    ...
    parseGatewayUrl(url) {
        try {
            ...
            throw new AnalyticsError(`Failed to parse gateway URL: ${error}`, 'INVALID_URL', 400, error);
        }
    }
    /**
    ...
    async executeQuery(query) {
        ...
        try {
            ...
            if (data.errors && data.errors.length > 0) {
                ...
                throw new AnalyticsError(error.message || 'GraphQL query failed', error.extensions?.code || 'GRAPHQL_ERROR', error.extensions?.code === 'authz' ? 403 : 500);
            }
            return {
                data,
                responseTime,
     ...
     */
    buildQuery(type, timeRange, filterId) {
        const variables = {
     ...
     */
    processAnalyticsResponse(result, timeRange) {
     ...
     * @param days - Number of days to query (optional, defaults to 30 days due to API limits)
     */
    async getChatAnalytics(chatId, days) {
