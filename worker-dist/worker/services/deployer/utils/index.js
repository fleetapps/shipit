 ...
 */
export async function createAssetManifest(files) {
    const manifest = {};
 ...
 */
export function mergeMigrations(migrations) {
 ...
 */
export function extractDurableObjectClasses(mergedMigration) {
    if (!mergedMigration)
        return [];
    return [
 ...
 */
export function buildWorkerBindings(config, hasAssets = false) {
    const bindings = [];
