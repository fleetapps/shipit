...
export async function proxyToAiGateway(request, env, _ctx) {
    ...
    try {
        ...
        const { baseURL, apiKey, defaultHeaders } = await getConfigurationForModel(modelName, env, app.userId);
...
export async function generateAppProxyToken(appId, userId, env, expiresInSeconds = 3 * 60 * 60 // 3 hours
) {
    const jwtSecret = new TextEncoder().encode(env.AI_PROXY_JWT_SECRET);
...
export function generateAppProxyUrl(env) {
    let protocol = 'https';
