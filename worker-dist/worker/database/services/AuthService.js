...
export class AuthService extends BaseService {
    ...
    async getOauthProvider(provider, request) {
     ...
     */
    async getOAuthAuthorizationUrl(provider, request, intendedRedirectUrl) {
     ...
     */
    async handleOAuthCallback(provider, code, state, request) {
        try {
            const oauthProvider = await this.getOauthProvider(provider, request);
            if (!oauthProvider) {
                throw new SecurityError(SecurityErrorType.INVALID_INPUT, `OAuth provider ${provider} not configured`, 400);
     ...
     */
    async findOrCreateOAuthUser(provider, oauthUserInfo) {
     ...
     */
    async logAuthAttempt(identifier, attemptType, success, request) {
        try {
     ...
     */
    validateRedirectUrl(redirectUrl, request) {
        try {
     ...
     */
    async generateAndStoreVerificationOtp(email) {
     ...
     */
    async verifyEmailWithOtp(email, otp, request) {
        try {
            // Find valid OTP
     ...
     */
    async validateTokenAndGetUser(token, env) {
        try {
