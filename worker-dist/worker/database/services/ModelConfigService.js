...
import { userModelConfigs } from '../schema';
...
export class ModelConfigService extends BaseService {
     ...
     */
    async getUserModelConfigs(userId) {
        ...
        for (const [actionKey, defaultConfig] of Object.entries(AGENT_CONFIG)) {
            ...
            if (userConfig) {
                ...
                result[actionKey] = {
                    name: userConfig.modelName ?? defaultConfig.name,
     ...
     */
    async getUserModelConfig(userId, agentActionName) {
        const userConfig = await this.database
            .select()
            .from(userModelConfigs)
            .where(and(eq(userModelConfigs.userId, userId), eq(userModelConfigs.agentActionName, agentActionName), eq(userModelConfigs.isActive, true)))
            .limit(1);
     ...
     */
    async getRawUserModelConfig(userId, agentActionName) {
        const userConfig = await this.database
            .select()
            .from(userModelConfigs)
            .where(and(eq(userModelConfigs.userId, userId), eq(userModelConfigs.agentActionName, agentActionName), eq(userModelConfigs.isActive, true)))
     ...
     */
    async upsertUserModelConfig(userId, agentActionName, config) {
     ...
     */
    async deleteUserModelConfig(userId, agentActionName) {
     ...
     */
    async resetAllUserConfigs(userId) {
        const result = await this.database
            .delete(userModelConfigs)
