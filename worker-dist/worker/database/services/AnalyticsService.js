...
export class AnalyticsService extends BaseService {
    ...
    async calculateUserStreak(userId) {
        try {
            ...
            const activities = await this.database
                .select({
                date: sql `DATE(${schema.apps.updatedAt})`
            })
                .from(schema.apps)
                .where(eq(schema.apps.userId, userId))
    ...
    async getUserActivityTimeline(userId, limit = 20) {
        ...
        const appActivities = await readDb
            ...
            action: sql `CASE WHEN ${schema.apps.createdAt} = ${schema.apps.updatedAt} THEN 'created' ELSE 'updated' END`,
