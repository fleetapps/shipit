...
export class SecretsService extends BaseService {
    ...
    async decryptSecret(encryptedValue) {
        try {
            ...
            const combined = new Uint8Array(Array.from(atob(encryptedValue), c => c.charCodeAt(0)));
     ...
     */
    async storeSecret(_userId, _secretData) {
    ...
    async getUserSecrets(userId) {
        try {
            const secrets = await this.database
                ...
                .where(and(eq(schema.userSecrets.userId, userId), eq(schema.userSecrets.isActive, true)))
     ...
     */
    async getAllUserSecrets(userId) {
        try {
            const secrets = await this.database
                .select()
                .from(schema.userSecrets)
     ...
     */
    async getSecretValue(userId, secretId) {
        try {
            const secret = await this.database
                .select()
                .from(schema.userSecrets)
                .where(and(eq(schema.userSecrets.id, secretId), eq(schema.userSecrets.userId, userId), eq(schema.userSecrets.isActive, true)))
                .get();
            if (!secret) {
     ...
     */
    async deleteSecret(_userId, _secretId) {
        // DISABLED: BYOK Disabled for security reasons
        throw new Error('BYOK is not supported for now');
        // try {
        //     await this.database
     ...
     */
    async getUserBYOKKeysMap(userId) {
        try {
     ...
     */
    async toggleSecretActiveStatus(_userId, _secretId) {
        // DISABLED: BYOK Disabled for security reasons
        throw new Error('BYOK is not supported for now');
        // try {
