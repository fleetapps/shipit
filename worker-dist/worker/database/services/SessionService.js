...
export class SessionService extends BaseService {
     ...
     */
    async logSecurityEvent(userId, sessionId, eventType, details, request) {
        try {
    ...
    async getUserSessions(userId) {
        try {
            const sessions = await this.db.db
                ...
                .where(and(eq(schema.sessions.userId, userId), eq(schema.sessions.isRevoked, false), gt(schema.sessions.expiresAt, new Date())))
    ...
    async getUserSecurityStatus(userId) {
        try {
            ...
            const recentEvents = await this.db.db
                ...
                .where(and(eq(schema.auditLogs.userId, userId), eq(schema.auditLogs.entityType, 'session'), gt(schema.auditLogs.createdAt, oneDayAgo)))
     ...
     */
    async revokeSessionId(sessionId) {
        try {
            await this.db.db
                .update(schema.sessions)
                .set({
     ...
     */
    async forceLogoutAllOtherSessions(userId, currentSessionId) {
        try {
